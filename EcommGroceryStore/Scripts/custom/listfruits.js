...
function getProducts(sort, pagesize, index, all, min, max, querystring, sortDirection) {
  //  console.log('i, getFruits, got called parameters passed are ' + sort + ' ' + pagesize + ' ' + index + ' ' + all + ' ' + min + ' ' + max);
    ...
    return jQuery.getJSON("/api/ProductDetails/getFruitsListWithSummary?sort=" + sort + "&pagesize=" + pagesize + "&index=" + index + "&all=" + all + "&min=" + min + "&max=" + max  + "&querystring=" + querystring + "&sortDirection=" + sortDirection + "",
       function (data) {
       });
}
